...
class ATest {
  ...
  static init(){
    this.tests = []
    this.page_name = window.location.pathname.split("/").filter(a => a != "").join("/")
   ...
   * unique du test)
   * 
   * @param {String}    titre Le titre du test
   * @param {String}    id Identifiant pour le test [OPTIONNEL]
   * @param {Function}  callback La fonction qui sera jouée pour jouer le test.
   * @param {Object}    data Les données transmises (options)
   * @param {Boolean}   data.tested  Si true et qu'on filtre seulement les tests à jouer, ce test-là sera à jouer.
   * @param {Boolean}   data.pending Si true, ce test est passé (à programmer plus tard)
   * @param {Boolean}   data.verbose  Si true, on fait un retour bavard de ce test.
   * 
   * @public
   */
  static add(titre, id, callback, data){
    ...
    id = id || "test-ligne"
    // On ajoute toujours le numéro de la ligne à l'identifant
    id = `${id}-L${data.line}`
    console.info("Test mémorisé (titre et id)", titre, id)
  ...
  /**
   * Récupère le numéro de ligne de la définition du test
   * 
   * @param {Error} erreur L'erreur générée (volontaire) pour l'obtenir
   * @return {Number} Le numéro de la ligne du test
   * 
   * @protected
   */
   ...
   */
  static run(){
    console.info("--- LANCEMENT DES TESTS DE LA PAGE --- (ATest.run)")
    var montest;
    /**
     * La première chose à faire, si nécessaire, est d'effectuer le
     * check qui est peut-être défini (souvent, une nouvelle page est
     * appelée avec un test précis à faire sur elle).
     */

    if ( montest = unstore("TODO-ATEST") ) {
      console.info("TEST MÉMORISÉ (à effectuer maintenant) : ", montest)
    }

    /**
     * Boucle sur tous les tests de la page
     */
    console.info("BOUCLE SUR LES %s TESTS DE LA PAGE %s", this.tests.length, this.page_name)
    this.tests.forEach(test => test.run())
    console.info("FIN DE BOUCLE SUR LES TESTS DE LA PAGE %s", this.page_name)

    /* Si on est encore là, c'est qu'aucun des tests précédents ne
     * chargeait une autre page. On a donc terminé les tests.
     * Noter que dans ce cas, la boucle ci-dessus est interrompue par
     * le rechargement de page et qu'on n'atteint donc jamais ce 
     * stop.
     */
    this.stop()
  }
  ...
  /**
   * Quand tous les tests de la page ont été joués.
   * Noter que cela est très loin d'arriver systématiquement
   */
  static stop(){
    console.info("=== FIN DES TESTS DE LA PAGE '%s' === (ATest.stop)", this.page_name)
    TestWorker.setFin()
...
/**
 * Au chargement de la page, on initialise toujour Alors-test
 */
ATest.init()
