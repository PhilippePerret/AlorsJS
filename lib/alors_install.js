window.Alors = (function(){
  ...
  const LIB_URL = 'https://www.atelier-icare.net/js-libraries/';
    ...
    static loadScriptsWithTags(local = false){
      ...
      WANTED_SCRIPTS.forEach(affix => {
        DCreate('SCRIPT', {
            class: 'alors-library'
          , in: 'document.head'
          , src: (local ? '/priv/static/js/' : LIB_URL) + `${affix}.js`
          , onerror: this.onLoadScriptError.bind(this)
          , onload:  this.onLoadScriptSuccess.bind(this)
        })
      })
    }
    /** 
